#####
# SG Bastion Host configuration v.1
#####
resource "aws_launch_template" "bastionhost" {
  ...
  user_data = var.userdata_file_content != "" ? base64encode(var.userdata_file_content) : base64encode(templatefile("${path.module}/atf-bastion-userdata.sh", { HOSTED_ZONE_ID = var.hosted_zone_id, NAME_PREFIX = var.name_prefix }))
...
resource "aws_autoscaling_group" "bastion-asg" {
  ...
  depends_on = [aws_launch_template.bastionhost]
}
